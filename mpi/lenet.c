...
uint8 PredictLabel(LeNet5 *lenet, image input, uint8 count) {
    Feature features = {0};
...
void TrainBatch(LeNet5 *lenet, image *inputs, uint8 *labels, int batchSize) {
    ...
    MPI_Scatter(inputs, localBatchSize * sizeof(image), MPI_BYTE,
                localInputs, localBatchSize * sizeof(image), MPI_BYTE, 0, MPI_COMM_WORLD);
    ...
    MPI_Reduce(buffer, globalBuffer, GETCOUNT(LeNet5), MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
...
int PredictBatch(LeNet5 *lenet, image *test_data, uint8 *test_label, int total_size) {
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    MPI_Scatter(test_data, localSize * sizeof(image), MPI_BYTE,
                localTestData, localSize * sizeof(image), MPI_BYTE, 0, MPI_COMM_WORLD);
    ...
    int globalCorrect = 0;
    MPI_Reduce(&localCorrect, &globalCorrect, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
